...
class EB_ORCA(PackedBinary, MakeCp):
    ...
    def install_step(self):
        ...
        if not self.cfg['files_to_copy']:
            ...
            # Version 6 from ORCACompoundMethods to CompoundScripts 
            if LooseVersion(self.version) >= LooseVersion('6.0.0'):
                compoundscripts = (['CompoundScripts'], 'bin')
                files_to_copy.append(compoundscripts)
             # Version 6 extra files lib 
            if LooseVersion(self.version) >= LooseVersion('6.0.0'):
                library = (['lib'], 'bin')
                files_to_copy.append(library)
             # Version 6 extra files  datasets
            if LooseVersion(self.version) >= LooseVersion('6.0.0'):
                datasets = (['datasets'], 'bin')
